...
`manifest.linuxBefore` array Annex B of `proton-install-core-spec.md` defines. The copy step's
writer (wave 2, u3) must yield `record.json` from `tree/`; the restore sweep's reader (wave 2,
u7) reads `record.json`; the pair is the one fixture the two agree by.

## The shape

One entry per file, directory, symbolic link or other entry under the game folder, the
